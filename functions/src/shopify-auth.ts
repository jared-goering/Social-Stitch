...
export const shopifyAuthCallback = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    console.log('[shopifyAuthCallback] Token exchange successful, got access token:', 
      tokenData.access_token ? `${tokenData.access_token.substring(0, 10)}...` : 'NONE');
...
export async function verifyRequestSession(req: functions.https.Request): Promise<{
  ...
  if (shopParam) {
    ...
    if (!storeDoc.exists || !storeDoc.data()?.accessToken) {
...
export const shopifyCheckInstall = functions.https.onRequest((req, res) => {
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      if (storeData?.accessTokenInvalid) {
